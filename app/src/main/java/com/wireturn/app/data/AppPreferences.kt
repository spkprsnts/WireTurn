    ...
    override fun <T> create(gson: Gson, type: TypeToken<T>): TypeAdapter<T>? {
        ...
        return object : TypeAdapter<T>() {
            ...
            override fun read(reader: JsonReader): T? {
                ...
                return try { delegate.read(JsonReader(StringReader("\"$name\""))) } catch (_: Exception) { constants.firstOrNull() }
            }
        }.nullSafe()
    }
}
...
data class TurnableConfig(
    ...
    fun isValid(): Boolean = username.isNotBlank() && platformId.isNotBlank() && callId.isNotBlank() && gateway.isNotBlank() && routes.isNotEmpty()

    fun toUrl(onlySelected: Boolean = false): String {
        val targetRoutes = if (onlySelected && selectedRouteId.isNotBlank()) routes.filter { it.routeId == selectedRouteId } else routes
        val builder = Uri.Builder().scheme("turnable")
        val userInfo = if (!userUuid.isNullOrBlank()) "${Uri.encode(userUuid)}:${Uri.encode(callId)}" else Uri.encode(callId)
            ...
            return try {
                ...
                val routes = pathParts.mapIndexed { index, routeId ->
                    TurnableRoute(
                        ...
                        socket = uri.getQueryParameter("socket[${index+1}]") ?: (if (index == 0) uri.getQueryParameter("socket") else null) ?: "udp",
                        transport = uri.getQueryParameter("transport[${index+1}]") ?: (if (index == 0) uri.getQueryParameter("transport") else null)
                    )
                }
                base.copy(
                    userUuid = userUuid ?: base.userUuid, callId = callId.ifBlank { base.callId },
                    platformId = uri.host ?: base.platformId, username = uri.getQueryParameter("username") ?: base.username,
                    type = uri.getQueryParameter("type") ?: "relay", encryption = uri.getQueryParameter("encryption") ?: base.encryption,
                    pubKey = uri.getQueryParameter("pub_key") ?: base.pubKey, peers = uri.getQueryParameter("peers")?.toIntOrNull() ?: base.peers,
                    forceTurn = uri.getQueryParameter("forceturn")?.toBoolean() ?: base.forceTurn, gateway = uri.getQueryParameter("gateway") ?: base.gateway,
                    proto = uri.getQueryParameter("proto") ?: base.proto, routes = routes.ifEmpty { base.routes },
                    selectedRouteId = uri.getQueryParameter("selected_route_id") ?: (if (routes.isNotEmpty()) routes.firstOrNull()?.routeId ?: "" else base.selectedRouteId)
                )
...
data class OlcrtcConfig(
    ...
    fun toUri(): String {
        ...
        when (transport) {
            "vp8channel" -> { if (vp8Fps != 60) params.add("vp8-fps=$vp8Fps"); if (vp8Batch != 64) params.add("vp8-batch=$vp8Batch") }
            "seichannel" -> { if (seiFps != 60) params.add("fps=$seiFps"); if (seiBatch != 64) params.add("batch=$seiBatch"); if (seiFrag != 900) params.add("frag=$seiFrag"); if (seiAckMs != 2000) params.add("ack-ms=$seiAckMs") }
            "videochannel" -> {
                if (videoW != 1080) params.add("video-w=$videoW"); if (videoH != 1080) params.add("video-h=$videoH"); if (videoFps != 60) params.add("video-fps=$videoFps")
                if (videoBitrate != "5000k") params.add("video-bitrate=$videoBitrate"); if (videoHw != "none") params.add("video-hw=$videoHw"); if (videoCodec != "qrcode") params.add("video-codec=$videoCodec")
                if (videoCodec == "qrcode") { if (videoQrSize != 0) params.add("video-qr-size=$videoQrSize"); if (videoQrRecovery != "low") params.add("video-qr-recovery=$videoQrRecovery") }
                else if (videoCodec == "tile") { if (videoTileModule != 4) params.add("video-tile-module=$videoTileModule"); if (videoTileRs != 20) params.add("video-tile-rs=$videoTileRs") }
            }
        }
    ...
    companion object {
        fun parse(url: String, base: OlcrtcConfig = OlcrtcConfig()): OlcrtcConfig? {
            ...
            return try {
                ...
                var cfg = base.copy(carrier = carrier, transport = transport, id = id, key = key, clientId = clientId.ifBlank { "wireturn" }, mimo = mimo)
                if (payload.isNotBlank()) {
                    val p = payload.split("&").associate { it.substringBefore("=") to it.substringAfter("=", "") }
                    cfg = when (transport) {
                        "vp8channel" -> cfg.copy(vp8Fps = p["vp8-fps"]?.toIntOrNull() ?: cfg.vp8Fps, vp8Batch = p["vp8-batch"]?.toIntOrNull() ?: cfg.vp8Batch)
                        "seichannel" -> cfg.copy(seiFps = p["fps"]?.toIntOrNull() ?: cfg.seiFps, seiBatch = p["batch"]?.toIntOrNull() ?: cfg.seiBatch, seiFrag = p["frag"]?.toIntOrNull() ?: cfg.seiFrag, seiAckMs = p["ack-ms"]?.toIntOrNull() ?: cfg.seiAckMs)
                        "videochannel" -> cfg.copy(videoW = p["video-w"]?.toIntOrNull() ?: cfg.videoW, videoH = p["video-h"]?.toIntOrNull() ?: cfg.videoH, videoFps = p["video-fps"]?.toIntOrNull() ?: cfg.videoFps, videoBitrate = p["video-bitrate"] ?: cfg.videoBitrate, videoHw = p["video-hw"] ?: cfg.videoHw, videoCodec = p["video-codec"] ?: cfg.videoCodec, videoQrSize = p["video-qr-size"]?.toIntOrNull() ?: cfg.videoQrSize, videoQrRecovery = p["video-qr-recovery"] ?: cfg.videoQrRecovery, videoTileModule = p["video-tile-module"]?.toIntOrNull() ?: cfg.videoTileModule, videoTileRs = p["video-tile-rs"]?.toIntOrNull() ?: cfg.videoTileRs)
                        else -> cfg
                    }
                }
                cfg
...
data class ClientConfig(
    ...
    fun getValidationErrorResId(): Int? = if (isRawMode && rawCommand.isBlank()) com.wireturn.app.R.string.error_raw_empty else if (!isRawMode && kernelVariant == KernelVariant.TURNABLE && !turnableConfig.isValid()) com.wireturn.app.R.string.error_settings_empty else if (!isRawMode && kernelVariant == KernelVariant.OLCRTC && !olcrtcConfig.isValid()) com.wireturn.app.R.string.error_settings_empty else null
...
data class XrayConfig(
    ...
    companion object { const val DEFAULT_SOCKS_BIND_ADDRESS = "127.0.0.1:1080" }
}
...
data class WgConfig(
    ...
    fun toWgString(): String = "[Interface]\nPrivateKey = $privateKey\nAddress = $address\nMTU = $mtu\n\n[Peer]\nPublicKey = $publicKey\nEndpoint = $endpoint\nPersistentKeepalive = $persistentKeepalive"
    companion object {
        fun parse(text: String): WgConfig {
            var pk = ""; var ad = ""; var m = ""; var pub = ""; var ep = ""; var pkp = ""
            var sec = ""
            text.lineSequence().forEach { l ->
                val t = l.trim()
                if (t.startsWith("[")) sec = t.lowercase()
                else if (t.contains("=")) {
                    val k = t.substringBefore("=").trim().lowercase()
                    val v = t.substringAfter("=").trim()
                    if (sec == "[interface]") when(k) { "privatekey" -> pk = v; "address" -> ad = v; "mtu" -> m = v }
                    else if (sec == "[peer]") when(k) { "publickey" -> pub = v; "endpoint" -> ep = v; "persistentkeepalive" -> pkp = v }
                }
            }
...
data class Profile(
    ...
    fun isEmpty(): Boolean = (rawCommand as Any?)?.toString().isNullOrBlank() && !turnableConfig.isValid() && !olcrtcConfig.isValid() && !wgConfig.isValid() && !vlessConfig.isValid()
...
data class GlobalVpnSettings(val hideSystemApps: Boolean = true, val bypassMode: Boolean = true, val filteringEnabled: Boolean = true, val groupAppsByLetter: Boolean = true)
data class AutoLaunchSettings(val enabled: Boolean = false, val checkUrl: String = "https://www.google.com", val intervalMinutes: Int = 15)
...
class AppPreferences(val context: Context) {
    ...
    private fun <T> Flow<Preferences>.mapPref(key: Preferences.Key<T>, def: T): Flow<T> = this.map { it[key] ?: def }.distinctUntilChanged()

    val onboardingDoneFlow: Flow<Boolean> = appCtx.internalDataStore.data.mapPref(ONBOARDING_DONE, false)
    suspend fun hasActiveProfile(): Boolean = appCtx.internalDataStore.data.map { it[ACTIVE_KERNEL_VARIANT] }.first() != null

    val themeModeFlow: Flow<ThemeMode> = appCtx.internalDataStore.data.map { ThemeMode.valueOf(it[THEME_MODE] ?: ThemeMode.DARK.name) }.distinctUntilChanged()
    ...
    val globalVpnSettingsFlow: Flow<GlobalVpnSettings> = appCtx.internalDataStore.data.map { GlobalVpnSettings(it[VPN_HIDE_SYSTEM_APPS] ?: true, it[VPN_BYPASS_MODE] ?: true, it[VPN_FILTERING_ENABLED] ?: true, it[VPN_GROUP_APPS_BY_LETTER] ?: true) }.distinctUntilChanged()
    val excludedAppsFlow: Flow<Set<String>> = appCtx.internalDataStore.data.map { it[XRAY_EXCLUDED_APPS] ?: emptySet() }.distinctUntilChanged()
    val profilesFlow: Flow<List<Profile>> = appCtx.internalDataStore.data.map { p -> 
        val json = p[PROFILES_JSON] ?: "[]"
        val list = try { gson.fromJson<List<Profile>>(json, object : TypeToken<List<Profile>>() {}.type) ?: emptyList() } catch(_: Exception) { emptyList() }
    ...
    val currentProfileNameFlow: Flow<String?> = combine(profilesFlow, currentProfileIdFlow) { profiles, id -> profiles.find { it.id == id }?.name }
    val vlessLinkHistoryFlow: Flow<List<String>> = appCtx.internalDataStore.data.map { p -> (p[VLESS_LINK_HISTORY] ?: "").split("|").filter { it.isNotBlank() } }
    ...
    val autoLaunchSettingsFlow: Flow<AutoLaunchSettings> = appCtx.internalDataStore.data.map { AutoLaunchSettings(it[AUTO_LAUNCH_ENABLED] ?: false, it[AUTO_LAUNCH_URL] ?: "https://www.google.com", it[AUTO_LAUNCH_INTERVAL] ?: 15) }.distinctUntilChanged()

    val clientConfigFlow: Flow<ClientConfig> = appCtx.internalDataStore.data.map { p ->
        ClientConfig(
            ...
            kernelVariant = try { KernelVariant.valueOf(p[ACTIVE_KERNEL_VARIANT] ?: KernelVariant.TURNABLE.name) } catch(_: Exception) { KernelVariant.TURNABLE },
    ...
    val xrayConfigFlow: Flow<XrayConfig> = appCtx.internalDataStore.data.map { p ->
        XrayConfig(
            ...
            xrayConfiguration = try { XrayConfiguration.valueOf(p[ACTIVE_XRAY_CONFIG_TYPE] ?: XrayConfiguration.WIREGUARD.name) } catch(_: Exception) { XrayConfiguration.WIREGUARD }
        ).fillDefaults()
    }.distinctUntilChanged()

    val xraySettingsFlow: Flow<XraySettings> = appCtx.internalDataStore.data.map { XraySettings(it[ACTIVE_XRAY_ENABLED] ?: false) }.distinctUntilChanged()
    val wgConfigFlow: Flow<WgConfig> = appCtx.internalDataStore.data.map { (gson.fromJson(it[ACTIVE_WG_JSON] ?: "{}", WgConfig::class.java) ?: WgConfig()).fillDefaults() }.distinctUntilChanged()
    val vlessConfigFlow: Flow<VlessConfig> = appCtx.internalDataStore.data.map { (gson.fromJson(it[ACTIVE_VLESS_JSON] ?: "{}", VlessConfig::class.java) ?: VlessConfig()).fillDefaults() }.distinctUntilChanged()
    ...
    suspend fun saveOlcrtcSocks(addr: String, auth: Boolean, user: String, pass: String) {
        appCtx.internalDataStore.edit { p -> p[OLCRTC_SOCKS_ADDR] = addr; p[OLCRTC_SOCKS_AUTH_ENABLED] = auth; p[OLCRTC_SOCKS_USER] = user; p[OLCRTC_SOCKS_PASS] = pass }
    }

    suspend fun saveXrayGlobal(socks: String, http: String, auth: Boolean, user: String, pass: String) {
        appCtx.internalDataStore.edit { p -> p[XRAY_SOCKS_BIND] = socks; p[XRAY_HTTP_BIND] = http; p[XRAY_AUTH_ENABLED] = auth; p[XRAY_USER] = user; p[XRAY_PASS] = pass }
    }

    suspend fun saveClientListenAddr(addr: String) { appCtx.internalDataStore.edit { it[CLIENT_LISTEN_ADDR] = addr } }
    suspend fun saveProfiles(list: List<Profile>) { appCtx.internalDataStore.edit { it[PROFILES_JSON] = gson.toJson(list) } }
    suspend fun setVpnEnabled(v: Boolean) { appCtx.internalDataStore.edit { it[VPN_ENABLED] = v } }
    suspend fun setDynamicTheme(v: Boolean) { appCtx.internalDataStore.edit { it[DYNAMIC_THEME] = v } }
    suspend fun setThemeMode(m: ThemeMode) { appCtx.internalDataStore.edit { it[THEME_MODE] = m.name } }
    suspend fun setOnboardingDone(v: Boolean) { appCtx.internalDataStore.edit { it[ONBOARDING_DONE] = v } }
    suspend fun setAppLanguage(l: String) { appCtx.internalDataStore.edit { it[APP_LANGUAGE] = l } }
    suspend fun setShowFloatingActionButton(v: Boolean) { appCtx.internalDataStore.edit { it[SHOW_FLOATING_ACTION_BUTTON] = v } }
    suspend fun setBatteryNotificationDismissed(v: Boolean) { appCtx.internalDataStore.edit { it[BATTERY_NOTIFICATION_DISMISSED] = v } }
    suspend fun setAppsExclusionHintShown(v: Boolean) { appCtx.internalDataStore.edit { it[APPS_EXCLUSION_HINT_SHOWN] = v } }
    suspend fun setAllowUnstableUpdates(v: Boolean) { appCtx.internalDataStore.edit { it[ALLOW_UNSTABLE_UPDATES] = v } }
    suspend fun setWaitForNetwork(v: Boolean) { appCtx.internalDataStore.edit { it[WAIT_FOR_NETWORK] = v } }
    suspend fun setRestartOnNetworkChange(v: Boolean) { appCtx.internalDataStore.edit { it[RESTART_ON_NETWORK_CHANGE] = v } }
    suspend fun setCaptchaStyleMod(v: Boolean) { appCtx.internalDataStore.edit { it[CAPTCHA_STYLE_MOD] = v } }
    suspend fun setCaptchaForceTint(v: Boolean) { appCtx.internalDataStore.edit { it[CAPTCHA_FORCE_TINT] = v } }
    suspend fun addVlessLinkToHistory(l: String) { appCtx.internalDataStore.edit { p -> val h = p[VLESS_LINK_HISTORY]?.split("|")?.filter { it.isNotBlank() } ?: emptyList(); p[VLESS_LINK_HISTORY] = (listOf(l) + h.filter { it != l }).take(3).joinToString("|") } }
    suspend fun removeVlessLinkFromHistory(l: String) { appCtx.internalDataStore.edit { p -> p[VLESS_LINK_HISTORY] = (p[VLESS_LINK_HISTORY]?.split("|")?.filter { it.isNotBlank() && it != l } ?: emptyList()).joinToString("|") } }
    suspend fun saveGlobalVpnSettings(s: GlobalVpnSettings) { appCtx.internalDataStore.edit { it[VPN_HIDE_SYSTEM_APPS] = s.hideSystemApps; it[VPN_BYPASS_MODE] = s.bypassMode; it[VPN_FILTERING_ENABLED] = s.filteringEnabled; it[VPN_GROUP_APPS_BY_LETTER] = s.groupAppsByLetter } }
    suspend fun saveExcludedApps(s: Set<String>) { appCtx.internalDataStore.edit { it[XRAY_EXCLUDED_APPS] = s } }
    suspend fun updateAutoLaunchSettings(s: AutoLaunchSettings) { appCtx.internalDataStore.edit { it[AUTO_LAUNCH_ENABLED] = s.enabled; it[AUTO_LAUNCH_URL] = s.checkUrl; it[AUTO_LAUNCH_INTERVAL] = s.intervalMinutes } }
    
    suspend fun saveWgConfig(c: WgConfig) { appCtx.internalDataStore.edit { it[ACTIVE_WG_JSON] = gson.toJson(c) } }
    suspend fun saveXraySettings(s: XraySettings) { appCtx.internalDataStore.edit { it[ACTIVE_XRAY_ENABLED] = s.xrayEnabled } }
    suspend fun saveVlessConfig(c: VlessConfig) { appCtx.internalDataStore.edit { it[ACTIVE_VLESS_JSON] = gson.toJson(c) } }
    suspend fun saveXrayConfig(c: XrayConfig) { appCtx.internalDataStore.edit { it[XRAY_SOCKS_BIND] = c.socksBindAddress; it[XRAY_HTTP_BIND] = c.httpBindAddress; it[XRAY_AUTH_ENABLED] = c.isProxyAuthEnabled; it[XRAY_USER] = c.proxyUser; it[XRAY_PASS] = c.proxyPass; it[ACTIVE_XRAY_CONFIG_TYPE] = c.xrayConfiguration.name } }
    suspend fun saveClientConfig(c: ClientConfig) { appCtx.internalDataStore.edit { it[CLIENT_LISTEN_ADDR] = c.listenAddr; it[OLCRTC_SOCKS_ADDR] = c.socksAddr; it[OLCRTC_SOCKS_AUTH_ENABLED] = c.isSocksAuthEnabled; it[OLCRTC_SOCKS_USER] = c.socksUser; it[OLCRTC_SOCKS_PASS] = c.socksPass; it[ACTIVE_KERNEL_VARIANT] = c.kernelVariant.name; it[ACTIVE_IS_RAW] = c.isRawMode; it[ACTIVE_RAW_CMD] = c.rawCommand; it[ACTIVE_TURNABLE_JSON] = gson.toJson(c.turnableConfig); it[ACTIVE_OLCRTC_JSON] = gson.toJson(c.olcrtcConfig) } }

    suspend fun resetAll() { appCtx.internalDataStore.edit { it.clear() }; withContext(Dispatchers.IO) { File(appCtx.filesDir, "custom_core").delete() } }
