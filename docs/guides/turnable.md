...
1. **Создать профиль:** Скопируйте ссылку `turnable://` из шага 7 в буфер обмена. Нажмите на блок профиля → `+` → откроется экран «Добавить профиль». В группе **«Импорт профиля/подписки»** нажмите **«Вставить ссылку»** — тип конфига определится автоматически, введите название профиля в появившемся диалоге и подтвердите.

2. **Маршрут:** Откроется экран настройки Turnable с уже заполненными данными — в первом блоке выберите нужный маршрут: **WireGuard** или **VLESS**. Нажмите **«Далее»**.

3. **Настройка Xray:** Откроется экран Xray. Импортируйте основной VPN-конфиг, соответствующий выбранному маршруту: WireGuard → WG-конфиг, VLESS → ссылку в поле **«URI конфигурации»**. Используйте кнопки в верхней панели: QR-код или иконку импорта («Из буфера» / «Из файла»).
