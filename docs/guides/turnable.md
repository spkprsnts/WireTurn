...
1. **Создать профиль:** Нажмите на блок профиля → `+` → «Создать профиль». Откроется экран создания: введите название и в группе **«Импорт»** вставьте ссылку `turnable://` из шага 8 — тип конфига определится автоматически.
