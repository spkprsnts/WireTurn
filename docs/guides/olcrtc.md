...
3. **Настройка Xray (опционально):** Откроется экран Xray — выбор протокола здесь недоступен, доступна только форма **«URI конфигурации»** (поддерживаются ссылки VLESS, Trojan, Hysteria2). Xray используется для работы через VPN. Настройка нужна для режима **Dual-route** — автоматического переключения на прямое соединение, минуя туннель olcRTC, при доступности сервера. Для импорта конфига используйте кнопки в верхней панели: QR-код или иконку импорта («Из буфера» / «Из файла»). Без настройки Xray тоже работает.
