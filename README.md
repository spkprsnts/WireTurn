...
- **Dual-route (VLESS)** — интеллектуальная маршрутизация: автоматическое переключение на прямой адрес сервера (Direct Route) при его доступности, минуя TURN-туннель для снижения задержек.
...
- **Privacy Mode** — режим конфиденциальности, скрывающий чувствительные данные (ссылки, адреса, UUID).
...
4. **Настройка Xray (для WireGuard/VLESS):**
   ...
   - **Dual-route (для VLESS)**: включите этот режим и укажите прямой адрес сервера, чтобы приложение могло автоматически переключаться на прямое соединение при его доступности.
   - При несовпадении `Endpoint` в конфиге WireGuard с локальным адресом туннеля приложение предложит исправить это в один клик.
